    ...
    <div class="container">
        <div class="row">
            <div class="col-sm-12 cate_main">
                <h2>${category.name }</h2>
                <em class="emph_en">${category.engName }</em>
                <img src="/kakao/test/image/cate_main.jpg">
            </div>
        </div>
       <form class="form-inline" action="cate_list.kakao" id="send-form">
        <input type="hidden" id="" name="catno" value="${param.catno }" >
                	...
                	<c:choose>
                		<c:when test="${type eq 'B' }">
		                    <ul class="tab_friends">
		                        <li><a href="/kakao/product/cate_list.kakao?catno=${param.catno }&type=B" class="link_tab">전체</a></li>
                					<c:forEach var="cat" items="${Categories }">
			                    	    <li><a href="/kakao/product/cate_list.kakao?catno=${cat.no }&type=S"  class="link_tab ${param.catno eq cat.no ? 'hightlight' : ''}">${cat.name }</a></li>
                					</c:forEach>
		                    </ul>
                		</c:when>
                		<c:when test="${type eq 'S' }">
	                    	<ul class="tab_friends">
		                        <li><a href="/kakao/product/cate_list.kakao?catno=${parentNo }&type=B" class="link_tab">전체</a></li>
                					<c:forEach var="cat" items="${Categories }">
										<li><a href="/kakao/product/cate_list.kakao?catno=${cat.no }&type=S"  class="link_tab ${param.catno eq cat.no ? 'hightlight' : ''}">${cat.name }</a></li>
                					</c:forEach>
		                    </ul>
        ...
        <div class="row head_product">
            ...
            <div class="col-sm-3">
	                <input type="checkbox" id="checkDelivery" name="global" value="Y" class="inp_g" onchange="sendform(1);" ${not empty param.global ? 'checked=checked' : '' }>글로벌 배송 기능 상품보기
            </div>
	            <div class="col-sm-6 text-right">
	                <div class="btn-group">
	                  <select class="form-control" name="character" onchange="sendform(1);">
	                    <option value="">전체캐릭터</option>
	                    <option value="ryan" ${param.character eq 'ryan' ? "selected" : "" }>라이언</option>
	                    <option value="muzi" ${param.character eq 'muzi' ? "selected" : "" }>무지</option>
	                    <option value="apeach" ${param.character eq 'apeach' ? "selected" : "" }>어피치</option>
	                    <option value="prodo" ${param.character eq 'prodo' ? "selected" : "" }>프로도</option>
	                    <option value="neo" ${param.character eq 'neo' ? "selected" : "" }>네오</option>
	                    <option value="tube" ${param.character eq 'tube' ? "selected" : "" }>튜브</option>
	                    <option value="jayg" ${param.character eq 'jayg' ? "selected" : "" }>제이지</option>
	                    <option value="con" ${param.character eq 'con' ? "selected" : "" }>콘</option>
	                  </select>
	                </div>
	                <div class="btn-group">
